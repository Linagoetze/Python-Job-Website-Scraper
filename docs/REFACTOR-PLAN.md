...
**This is an archive, not a startup read.** The refactor is finished, and as of
SP0b (2026-09-11) its decisions log lives at `docs/DECISIONS.md` — the roughly
800 lines of this file a session actually needs — so nothing here is read at
the start of a session any more. Consult this file on demand: grep it for a
package name or a keyword when `docs/DECISIONS.md` points you here for the full
story, rather than reading it start to finish.
...
This file is two things: the record of how the project got from where "The big
picture" describes to where it is now, and — until SP0b — the decisions log,
which the audit called the most valuable thing in the repository. That log now
lives at `docs/DECISIONS.md`; see the pointer where it used to be, below. What
is left to do is under [Future work](#future-work), and it has been superseded
by `docs/SOURCES-PLAN.md`.
...
Moved to [`docs/DECISIONS.md`](DECISIONS.md) in SP0b (2026-09-11) — the log is
now a living file every session reads at the start, not an archive section.
Append new decisions there, not here. What follows below this point is the
per-package prompts and results this archive exists to hold.
...
**Superseded by `docs/SOURCES-PLAN.md`, in SP0b (2026-09-11).** What was
planned here as two open-ended items is now three concrete packages there:
**SP4** (fixture coverage for the five uncovered generic-ATS readers), **SP5**
(add the new companies), and **SP6** (fixtures for the remaining eight
readers). Read that file rather than this section — it carries the same
method (capture first, then read the extractor) and the same reasoning about
why an uncovered reader is an open question about correctness, not a
paperwork gap.

This section is left in place, shrunk, only so the anchor below does not
dangle for anything still linking to it.
